...
int oem_is_kcfactory(void)
{
	...
	if (status == -1) {
		status = (strstr(saved_command_line, "androidboot.mode=kcfactory") != NULL);
	}

	return status;
}
EXPORT_SYMBOL(oem_is_kcfactory);
...
int oem_is_off_charge(void)
{
	...
	if (status == -1) {
		status = (strstr(saved_command_line, "androidboot.mode=charger") != NULL);
	}
