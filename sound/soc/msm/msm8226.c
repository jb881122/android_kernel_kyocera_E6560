...
#ifdef CONFIG_KYOCERA_MSND
#define KC_LO_1_RCV_AMP   0x1
#define KC_LO_2_RCV_AMP   0x2
...
static int kc_ext_rcv_amp_gpio_gain = -1;
static int kc_ext_rcv_amp_gpio_sel  = -1;
...
static void kc_msm8226_ext_rcv_power_amp_on(u32 rcv)
{
	if ((gpio_is_valid(kc_ext_rcv_amp_gpio_on)) &&
		(gpio_is_valid(kc_ext_rcv_amp_gpio_gain)) &&
		(gpio_is_valid(kc_ext_rcv_amp_gpio_sel))) {
		if (rcv & (KC_LO_1_RCV_AMP | KC_LO_2_RCV_AMP)) {
			pr_debug("%s:Enable left and right receiver case spk = 0x%x\n",
				__func__, rcv);

			kc_msm8226_ext_rcv_pamp |= rcv;

			if ((kc_msm8226_ext_rcv_pamp & KC_LO_1_RCV_AMP) &&
				(kc_msm8226_ext_rcv_pamp & KC_LO_2_RCV_AMP)) {
			...
			struct snd_kcontrol *k, int event)
{
	...
	if (SND_SOC_DAPM_EVENT_ON(event)) {
		if (!strncmp(w->name, "Lineout_1 rcv amp", 18))
			kc_msm8226_ext_rcv_power_amp_on(KC_LO_1_RCV_AMP);
		else if (!strncmp(w->name, "Lineout_2 rcv amp", 18))
			kc_msm8226_ext_rcv_power_amp_on(KC_LO_2_RCV_AMP);
		else {
		...
		if (!strncmp(w->name, "Lineout_1 rcv amp", 18))
			kc_msm8226_ext_rcv_power_amp_off(KC_LO_1_RCV_AMP);
		else if (!strncmp(w->name, "Lineout_2 rcv amp", 18))
			kc_msm8226_ext_rcv_power_amp_off(KC_LO_2_RCV_AMP);
		else {
			pr_err("%s() Invalid Receiver Widget = %s\n",
				__func__, w->name);
			return -EINVAL;
...
#ifdef CONFIG_KYOCERA_MSND
	SND_SOC_DAPM_SPK("Lineout_1 rcv amp", kc_msm8226_ext_rcvamp_event),
	SND_SOC_DAPM_SPK("Lineout_2 rcv amp", kc_msm8226_ext_rcvamp_event),
...
static __devinit int msm8226_asoc_machine_probe(struct platform_device *pdev)
{
	...
	kc_ext_rcv_amp_gpio_sel = of_get_named_gpio(pdev->dev.of_node,
				"kc,rcv-amp-gpio-sel", 0);
	if (kc_ext_rcv_amp_gpio_on < 0) {
		dev_err(&pdev->dev, "Looking up %s property %d\n", pdev->dev.of_node->full_name, kc_ext_rcv_amp_gpio_sel);
	} else {
		ret = gpio_request(kc_ext_rcv_amp_gpio_sel, "RECEIVER_AMP_SEL");
		if (ret) {
			dev_err(card->dev,
					"%s: Failed to request rcvr gpio sel %d\n", __func__, kc_ext_rcv_amp_gpio_sel);
			goto err_lineout_spkr;
		}
	}

	kc_ext_rcv_amp_gpio_gain = of_get_named_gpio(pdev->dev.of_node,
				"kc,rcv-amp-gpio-gain", 0);
	if (kc_ext_rcv_amp_gpio_gain < 0) {
		dev_err(&pdev->dev, "Looking up %s property %d\n", pdev->dev.of_node->full_name, kc_ext_rcv_amp_gpio_gain);
	} else {
		ret = gpio_request(kc_ext_rcv_amp_gpio_gain, "RECEIVER_AMP_GAIN");
		if (ret) {
			dev_err(card->dev,
					"%s: Failed to request rcvr gpio gain %d\n", __func__, kc_ext_rcv_amp_gpio_gain);
			goto err_lineout_spkr;
		}
	}
...
err_lineout_spkr:
	...
	if (kc_ext_rcv_amp_gpio_sel >= 0) {
		gpio_free(kc_ext_rcv_amp_gpio_sel);
		kc_ext_rcv_amp_gpio_sel = -1;
	}
	if (kc_ext_rcv_amp_gpio_gain >= 0) {
		gpio_free(kc_ext_rcv_amp_gpio_gain);
		kc_ext_rcv_amp_gpio_gain = -1;
	}
	if (kc_ext_spk_amp_gpio_ctrl >= 0) {
