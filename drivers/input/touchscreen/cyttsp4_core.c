...
#define OEM_STATE_SLEEP_BIT	(1<<0) 
#define OEM_STATE_SWITCH_BIT	(1<<1) 


#define OEM_LIMIT_EVENT_NUM_IN_CALL	(2)
...
static long cyttsp4_ts_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case IOCTL_DIAG_INIT_TEST:
		...
		kfree(retdata);
		break;
	case IOCTL_CHANGE_LCD_STATUS:
		dev_dbg(dev, "%s: IOCTL_CHANGE_LCD_STATUS\n", __func__);
		if (copy_from_user(&lcd_stat, (void __user *)arg, sizeof(bool))) {
			rc = -EFAULT;
