...
int32_t msm_sensor_cci_clear_i2c_status( struct msm_camera_i2c_client *client )
{
    int32_t rc = -EFAULT;
    struct msm_camera_cci_ctrl cci_ctrl;
    ...
    if( client == NULL )
    {
        pr_err("Parm NULL Err\n");
        return rc;
    }
    
    cci_ctrl.cmd = MSM_CCI_CLEAR_I2C_STATUS;
    cci_ctrl.cci_info = client->cci_client;
    rc = v4l2_subdev_call(client->cci_client->cci_subdev,
        core, ioctl, VIDIOC_MSM_CCI_CFG, &cci_ctrl);
    if (rc < 0) {
...
int32_t msm_sensor_cci_get_i2c_status( struct msm_camera_i2c_client *client )
{
    int32_t rc = -EFAULT;
    struct msm_camera_cci_ctrl cci_ctrl;
    ...
    cci_ctrl.cmd = MSM_CCI_GET_I2C_STATUS;
    cci_ctrl.cci_info = client->cci_client;
    rc = v4l2_subdev_call(client->cci_client->cci_subdev,
        core, ioctl, VIDIOC_MSM_CCI_CFG, &cci_ctrl);
    if (rc < 0) {
