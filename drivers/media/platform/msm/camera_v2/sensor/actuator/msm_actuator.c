...
static int32_t msm_actuator_platform_probe(struct platform_device *pdev)
{
  ...
  ret = msm_actuator_t->i2c_client.i2c_func_tbl->i2c_util(
  ...
  gpio_request( 36, "CAM_STANDBY");
  
  gpio_direction_output( 36, 1 );
  
  msm_actuator_t->i2c_client.addr_type = MSM_ACTUATOR_BYTE_ADDR;
  for( i = 0; i < 3; i++ ) {
  
    ret = msm_sensor_cci_clear_i2c_status( &msm_actuator_t->i2c_client );
    if( ret < 0 ) {
      pr_err("CCI I2C Status Clear Err:%d\n", ret);
    }
    
    ret = msm_camera_cci_i2c_write( &msm_actuator_t->i2c_client, 0x00, 0x00, 
                                    MSM_ACTUATOR_BYTE_DATA );
    if ( ret >= 0 ) {
      ret = msm_sensor_cci_get_i2c_status( &msm_actuator_t->i2c_client );
      if ( ret >= 0 ) {
        pr_err("AF Module MCNEX\n");
        break;
      }
    }
    else {
      pr_err("%s AF Module I2C Err\n", __func__ );
    }
  }
  
  if( i == 3 ) {
    pr_err("AF Module Sunny\n");
    msm_actuator_t->cam_name = 3;
