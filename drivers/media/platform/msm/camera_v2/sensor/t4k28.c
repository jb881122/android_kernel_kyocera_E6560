...
static int32_t fps_mode = MSM_CAMERA_FRAMERATE_MODE_AUTO;
...
static struct msm_camera_i2c_reg_conf t4k28_framerate_720p_fix15_settings[] = {
    {0x351B, 0x18},
};
...
static struct msm_camera_i2c_reg_conf t4k28_framerate_fix15_settings[] = {
    {0x351B, 0x18},
};
...
static long t4k28_set_framerate_mode(struct msm_sensor_ctrl_t *s_ctrl, int32_t mode)
{
    ...
    fps_mode = mode;
	CDBG("%s res %d fpsmode %d", __func__, res_mode, fps_mode);

    switch(res_mode) {
    case 1:
        switch (mode) {
        case MSM_CAMERA_FRAMERATE_MODE_AUTO:
            data = &t4k28_framerate_720p_auto_settings[0];
            size = ARRAY_SIZE(t4k28_framerate_720p_auto_settings);
            break;
        case MSM_CAMERA_FRAMERATE_MODE_FIX:
            data = &t4k28_framerate_720p_fix30_settings[0];
            size = ARRAY_SIZE(t4k28_framerate_720p_fix30_settings);
            break;
        case MSM_CAMERA_FRAMERATE_MODE_FIX15:
            data = &t4k28_framerate_720p_fix15_settings[0];
            size = ARRAY_SIZE(t4k28_framerate_720p_fix15_settings);
            break;
        default:
            break;
        }
    ...
    case MSM_SENSOR_RES_FULL:
        ...
        case MSM_CAMERA_FRAMERATE_MODE_AUTO:
            ...
            break;
        case MSM_CAMERA_FRAMERATE_MODE_FIX:
            data = &t4k28_framerate_fix30_settings[0];
            size = ARRAY_SIZE(t4k28_framerate_fix30_settings);
            break;
        case MSM_CAMERA_FRAMERATE_MODE_FIX15:
            data = &t4k28_framerate_fix15_settings[0];
            size = ARRAY_SIZE(t4k28_framerate_fix15_settings);
            break;
        default:
            break;
        }
        break;
    case MSM_SENSOR_INVALID_RES:
    default:
        break;
    }
    if (data) {
        ...
        if ( wb_read ) {
{
            ...
            t4k28_write_exposure_value[0].reg_data = (tmp_data >> 8);
            t4k28_write_exposure_value[1].reg_data = tmp_data & 0xFF;
            t4k28_write_exposure_value[2].reg_data = t4k28_keep_exposure_value[2].reg_data & 0x0F;
            t4k28_write_exposure_value[3].reg_data = t4k28_keep_exposure_value[3].reg_data;
