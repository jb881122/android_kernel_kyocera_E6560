			...
			struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	...
	if (disp_ext_panel_get_type() != PANEL_TYPE_G) {
		if(OEM_get_board() >= OEM_BOARD_PP_TYPE){
			...
			printk("[Display]%s() TYPE T / OTP ON \n", __func__);
		}else if(OEM_get_board() >= OEM_BOARD_LAB_TYPE){
			mdss_dsi_parse_dcs_cmds(np, &ctrl_pdata->on_cmds,
				"qcom,mdss-dsi-on-command-lab", "qcom,mdss-dsi-on-command-state");
			printk("[Display]%s() TYPE T / OTP ON(Lab) \n", __func__);
		}else if(OEM_get_board() >= OEM_BOARD_WS2_2_TYPE){
			mdss_dsi_parse_dcs_cmds(np, &ctrl_pdata->on_cmds,
				"qcom,mdss-dsi-on-command-ws2-2", "qcom,mdss-dsi-on-command-state");
			printk("[Display]%s() TYPE T / OTP ON(WS2-2) \n", __func__);
		}else{
			mdss_dsi_parse_dcs_cmds(np, &ctrl_pdata->on_cmds,
				"qcom,mdss-dsi-on-command-otpoff", "qcom,mdss-dsi-on-command-state");
			printk("[Display]%s() TYPE T / OTP OFF \n", __func__);
		}
		mdss_dsi_parse_dcs_cmds(np, &ctrl_pdata->off_cmds,
			"qcom,mdss-dsi-off-command", "qcom,mdss-dsi-off-command-state");
	} else {
		mdss_dsi_parse_dcs_cmds(np, &ctrl_pdata->on_cmds,
			"qcom,mdss-dsi-on-command_prototype", "qcom,mdss-dsi-on-command-state");

		printk("[Display]%s() TYPE G \n", __func__);
		mdss_dsi_parse_dcs_cmds(np, &ctrl_pdata->off_cmds,
			"qcom,mdss-dsi-off-command_prototype", "qcom,mdss-dsi-off-command-state");
	}
