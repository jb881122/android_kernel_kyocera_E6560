...
extern void  disp_ext_panel_bl_ctrl_direct( struct mdss_panel_data *pdata, u32 bl_level );
extern void  disp_ext_invert_ctrl_direct(struct mdss_panel_data *pdata, int data);
extern void  disp_ext_color_adjustment_ctrl_direct(struct mdss_panel_data *pdata, int enable, int red, int green, int blue);
...
#if 1 
  if(oem_is_kcfactory()){
	printk("[Display]%s(Disp off) \n", __func__);
	mdss_fb_set_backlight(mfd, 0);
	ret = mdss_fb_send_panel_event(mfd, MDSS_EVENT_BLANK, NULL);
	ret = mdss_fb_send_panel_event(mfd, MDSS_EVENT_PANEL_OFF, NULL);
	msleep(500);
	printk("[Display]%s(Disp on) \n", __func__);
	ret = mdss_fb_send_panel_event(mfd, MDSS_EVENT_RESET, NULL);
	if(ret){printk("%s() error:%d \n", __func__,ret );}
	ret = mdss_fb_send_panel_event(mfd, MDSS_EVENT_UNBLANK, NULL);
	if(ret){printk("%s() error:%d \n", __func__,ret );}
	ret = mdss_fb_send_panel_event(mfd, MDSS_EVENT_PANEL_ON, NULL);
	if(ret){printk("%s() error:%d \n", __func__,ret );}
	printk("[Display]%s(Disp backlight) \n", __func__);
	mdss_fb_set_backlight(mfd, 102);
  }
#endif
			 ...
			 unsigned long arg)
{
	...
	case MSMFB_DISP_MSG_OUT:
	{
		...
		break;
#endif /* CONFIG_DISP_EXT_DIAG */
	case MSMFB_COLOR_ADJ:
	{
		struct color_adjustment_data adjustment_info;
		struct mdss_panel_data *pdata;

		pdata = dev_get_platdata(&mfd->pdev->dev);
		ret = copy_from_user(&adjustment_info, argp, sizeof(struct color_adjustment_data));
		if (ret) {
			pr_err("%s:ioctl failed\n", __func__);
			return ret;
		}

		disp_ext_invert_ctrl_direct(pdata, adjustment_info.invert);
		disp_ext_color_adjustment_ctrl_direct(pdata, adjustment_info.enable, adjustment_info.red, adjustment_info.green, adjustment_info.blue);
	}
		break;
	default:
