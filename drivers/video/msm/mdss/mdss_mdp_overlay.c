						...
						int *pipe_ndx, int splash_event)
{
	...
	if (splash_event == MDP_CREATE_SPLASH_OV) {
		...
		if(oem_is_off_charge()){
			kernel_disp_raw_bitmap_SR(0,0,720,1280, kernel_offcharge_logo, fbi->screen_base, 1);
		}else{
			kernel_disp_raw_bitmap_SR(0,0,720,1280, splash_bgr888_image, fbi->screen_base, 0);
		}
#endif
