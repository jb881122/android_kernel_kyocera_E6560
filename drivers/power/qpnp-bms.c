...
#define OEM_UPPER_LIMIT_TEMP	400 
...
void oem_bms_chip_dump(const char* msg)
{
	if (!the_chip) {
		pr_err("the_chip is 0.\n");
		return;
	}

	printk("%s(%d):%s charge_cycles=%d\n", __func__, __LINE__, msg? msg: "", the_chip->charge_cycles);
	printk("chip:enable_fcc_learning=%d fcc_resolution=%d fcc_mah=%d fcc_new_mah=%d\n",
		the_chip->enable_fcc_learning,
		the_chip->fcc_resolution,
		the_chip->fcc_mah,
		the_chip->fcc_new_mah);

	printk("    :fcc_sample_count=%d min_fcc_learning_samples=%d\n",
		the_chip->fcc_sample_count,
		the_chip->min_fcc_learning_samples);

	printk("    :min_fcc_ocv_pc=%d fcc_new_batt_temp=%d \n",
		the_chip->min_fcc_ocv_pc,
		the_chip->fcc_new_batt_temp);
}
...
static void reset_for_new_battery(struct qpnp_bms_chip *chip, int batt_temp)
{
	...
	if (chip->enable_fcc_learning) {
		...
		pr_debug(">>backup_charge_cycle cycles=%d increase=%d\n",
			chip->charge_cycles,
							...
							int chargecycles)
{
	...
	pr_debug("qpnp_write_wrapper=%d fcc_new=%d fcc_mah=%d fcc_resolution=%d\n",
		rc, fcc_new, fcc_mah, chip->fcc_resolution);
...
#else/* CONFIG_OEM_BMS */
	if ((new_fcc_uah >= (fcc_uah / 2)) &&
		((new_fcc_uah * 100) <= (fcc_uah * 125)))
		return true;

	pr_err("FCC rejected - not within valid limit. new_fcc_uah=%d fcc_uah=%d\n",
			new_fcc_uah, fcc_uah);
#endif/* CONFIG_OEM_BMS */
	...
	if (start) {
		...
		if (is_new_fcc_valid(new_fcc_uah, fcc_uah)) {
			...
			&& batt_temp <= OEM_UPPER_LIMIT_TEMP) {
				pr_debug(">>update_fcc_learning_table batt_temp=%d\n", batt_temp);
				update_fcc_learning_table(
					chip,
					new_fcc_uah,
					chip->charge_cycles,
					batt_temp);
				...
				pr_err("batt_temp[%d] is outside of the rand[%d, %d].\n",
					batt_temp, OEM_LOWER_LIMIT_TEMP, OEM_UPPER_LIMIT_TEMP);
			}
		}
		else {
			pr_err("is_new_fcc_valid=0. new_fcc_uah=%d fcc_uah=%d batt_temp=%d\n",
				new_fcc_uah, fcc_uah, batt_temp);
		}
#endif/* CONFIG_OEM_BMS */
...
static void battery_status_check(struct qpnp_bms_chip *chip)
{
	...
	if (chip->battery_status != status) {
		...
		if (status == POWER_SUPPLY_STATUS_CHARGING) {
			...
			charging_began(chip);
            if (oem_chg_get_batt_property(POWER_SUPPLY_PROP_OEM_CHARGE_CONNECT_STATE) == POWER_SUPPLY_OEM_CONNECT_USB)
            {
                kdnand_emmc_refresh();
            }
...
static inline int bms_read_properties(struct qpnp_bms_chip *chip)
{
	...
	if (OEM_get_board() == OEM_BOARD_WS2_2_TYPE) {
		chip->use_external_rsense = 0;
		pr_info("board type:%d Use internal rsence\n", OEM_get_board());
	} else
