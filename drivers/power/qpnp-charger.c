...
#define FACTORY_CHG_VOLT_MV	3930
...
qpnp_chg_set_appropriate_vbatdet(struct qpnp_chg_chip *chip)
{
	...
	{
		qpnp_chg_vbatdet_set(chip, 4400);
	...
	if (chip->fastchg_on ^ fastchg_on) {
		...
		if (fastchg_on) {
			if (qpnp_chg_is_usb_chg_plugged_in(chip)) {
				...
				if ((ret.intval == POWER_SUPPLY_TYPE_USB_DCP) && oem_chg_get_usb_det_status()) {
...
switch_usb_to_charge_mode(struct qpnp_chg_chip *chip)
{
	...
	oem_chg_set_pma_en(0);
	pr_info("GPIO PMA_EN:0 USB_PATH_EN:0\n");
...
switch_usb_to_host_mode(struct qpnp_chg_chip *chip)
{
	...
	pr_info("GPIO PMA_EN:1 USB_PATH_EN:1\n");
...
static enum power_supply_property msm_batt_power_props[] = {
	...
	POWER_SUPPLY_PROP_OEM_REMAINING,
	POWER_SUPPLY_PROP_OEM_TEMP_STATUS,
	POWER_SUPPLY_PROP_OEM_CRITICAL,
...
*/
	...
	if ((oem_bms_get_batt_health_check_st() >= BATT_HEALTH_CHECK_GET) &&
		(oem_bms_get_batt_health_check_st() <= BATT_HEALTH_CHECK_CALC)) {
		return POWER_SUPPLY_STATUS_CHARGING;
	}
				       ...
				       union power_supply_propval *val)
{
  ...
  if (-EINVAL != oem_bms_pm_batt_power_get_property(psp, &(val->intval))) {
    return 0;
  }
...
qpnp_chg_set_appropriate_battery_current(struct qpnp_chg_chip *chip)
{
	...
	pr_info("not change setting to %dmA\n", chg_current);
}
...
#define VBATDET_MAX_ERR_COOL_MV	150
#define VBATDET_MAX_ERR_WARM_MV	150
static void
...
void oem_chg_ibatmax_set(int chg_current)
{
	qpnp_chg_ibatmax_set(the_chip, chg_current);
}
