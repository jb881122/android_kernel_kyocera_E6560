...
#define SET_PMA_EOC(val) gpio_set_value(GPIO_PMA_EOC, val)
...
static void oem_chg_usb_detection(struct work_struct *work)
{
	...
	if (USB_CHECK_COUNT <= check_cnt) {
		...
		pr_info("GPIO PMA_EN:%d USB_PATH_EN:%d\n", current_usb_det_status, current_usb_det_status);
...
void oem_chg_set_pma_eoc(bool value)
{
	SET_PMA_EOC(value);
	pr_info("PMA_EOC=%d\n", value);
}
EXPORT_SYMBOL(oem_chg_set_pma_eoc);
...
void oem_chg_det_init(void *chip)
{
	...
	gpio_request( GPIO_PMA_EOC, "GPIO_PMA_EOC PORT" );
	gpio_tlmm_config(GPIO_CFG(GPIO_PMA_EOC, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
