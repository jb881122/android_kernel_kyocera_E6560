				...
				const struct usb_ctrlrequest *ctrl)
{
	...
	if (ctrl->bRequestType ==
		...
		if ((ctrl->bRequest == 1)
			...
			head = (struct mtp_ext_config_desc_header *)cdev->req->buf;
			func = (struct mtp_ext_config_desc_function *)(head + 1);
			...
			list_for_each_entry(cfg, &cdev->configs, list) {
				list_for_each_entry(f, &cfg->functions, list) {
					...
					if (!strncmp(f->name, "mtp", 3)) {
						memcpy(func->compatibleID, "MTP", 3);
						VDBG(cdev, "MTP interface found."
							"Interface_num: %d.\n",
							interface_num);
					}
					func++;
