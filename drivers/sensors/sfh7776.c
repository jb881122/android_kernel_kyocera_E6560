...
#define SFH7776_PS_TL_DEFAULT	3
#define SFH7776_PS_TL_BASE	4
...
static struct proximity_threshold proximity_thres_tbl[] = {
     ...
     {1, 2, 22,    17 },
     {2, 2, 19,    9  },
     {3, 2, 10,    SFH7776_PS_TL_DEFAULT + SFH7776_PS_TL_BASE  },
     {4, 2, 8,     0  },
 };
...
static void sfh7776_restore_ps_thresh(void)
{
    memcpy(proximity_thres, proximity_thres_tbl_bk, sizeof(proximity_thres_tbl_bk));

    SFH7776_DEBUG_LOG("%s: Set Default ThresholdTable\n",__func__);
}
static void sfh7776_check_ps_thresh(void)
{
    ...
    for(i = 0; i <= proximity_max_level; ++i){
        if(proximity_thres[i].hi <= proximity_thres[i].lo){
            SFH7776_DEBUG_LOG("%s: Error thresh[%d] lo val:%d hi val:%d\n",
                              __func__, i, proximity_thres[i].hi, proximity_thres[i].lo);
            sfh7776_restore_ps_thresh();
            SFH7776_DEBUG_LOG("%s: [OUT]\n",__func__);
            return;
        }
        if(i != 0){
            if(proximity_thres[i].hi <= proximity_thres[i-1].lo){
                SFH7776_DEBUG_LOG("%s: Error thresh[%d].hi val:%d thresh[%d].hi val:%d\n",
...
static void sfh7776_ps_thresh_cal(struct sfh7776_data *data)
{
    SFH7776_DEBUG_LOG("%s[IN]\n", __func__);

    if(atomic_read(&g_ct_val) != 0)
    {
        SFH7776_DEBUG_LOG("%s: ps_thresh already changed\n",__func__);
        SFH7776_DEBUG_LOG("%s: [OUT]\n",__func__);
        return;
    }

    if(data->ps_data >=0 && data->ps_data < 4){
        proximity_thres[3].lo = data->ps_data + SFH7776_PS_TL_BASE;
        SFH7776_DEBUG_LOG("%s: Lev3 OFF thresh=%d\n",__func__, proximity_thres[3].lo);
        sfh7776_check_ps_thresh();
    }
    else {
        SFH7776_DEBUG_LOG("%s: out ps_data=%d\n",__func__, data->ps_data);
        sfh7776_restore_ps_thresh();
    }

    atomic_set(&g_ct_val, 1);
...
static unsigned long sfh7776_als_lux_conversion(unsigned int vis, unsigned int ir)
{
    ...
    if(307 > ratio)
    {
        lux = 12694*vis/GAIN_VIS - 28853*ir/GAIN_IR;
        lux = lux / 1000 * GA_A / 100;
    }
    else if(613 > ratio)
    {
        lux = 6015*vis/GAIN_VIS - 7130*ir/GAIN_IR;
        lux = lux / 1000 * GA_D / 100;
    }
    else if(850 > ratio)
    {
        lux = 3191*vis/GAIN_VIS - 2525*ir/GAIN_IR;
        lux = lux / 1000 * GA_E / 100;
    }
    else if(1639 > ratio)
    {
        lux = 1930*vis/GAIN_VIS - 1178*ir/GAIN_IR;
    ...
    {
        lux = 3860*vis/GAIN_VIS - 1390*ir/GAIN_IR;
        lux = lux / 1000 * GA_C / 100;
    }
    else if(4097 > ratio)
    {
        lux = 7720*vis/GAIN_VIS - 1566*ir/GAIN_IR;
        lux = lux / 1000 * GA_C / 100;
    }
    else
    {
        lux = 15441*vis/GAIN_VIS;
        lux = lux / 1000 * GA_C / 100;
    }
...
static struct attribute *sfh7776_attributes[] = {
...
static const struct attribute_group sfh7776_attr_group = {
    .attrs = sfh7776_attributes,
};
                   ...
                   const struct i2c_device_id *id)
{
    ...
    err = sysfs_create_group(&client->dev.kobj, &sfh7776_attr_group);
    if (err)
    {
        SFH7776_DEBUG_LOG("Failed sysfs_create_group\n");
...
exit_sysfs_remove:
    sysfs_remove_group(&client->dev.kobj, &sfh7776_attr_group);
