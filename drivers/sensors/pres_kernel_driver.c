...
#if PRESSURE_DEBUG
#define PRESS_DEBUG_LOGD( arg... )   printk("PRESS:" arg )
#else
#define PRESS_DEBUG_LOGD( arg... )
#endif
...
sensor_probe(struct platform_device *pdev)
{
	...
	if (!input_data) {
		rt = -ENOMEM;
		printk("sensor_probe: Failed to allocate input_data device\n");
		goto err;
	}
	...
	if (rt) {
		printk("sensor_probe: Unable to register input_data device: "
					"%s\n", input_data->name);
		goto err;
	}
...
MODULE_DESCRIPTION("T5403 pressure sensor class driver");
MODULE_LICENSE("GPL");
