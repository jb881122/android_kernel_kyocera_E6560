...
#define HSCDTD_CTRL1_ACTIVE	0x98
#define HSCDTD_CTRL1_INACTIVE	0x18
...
static int hscdtd_default_param(void)
{
	int err;
	u8 bufctl3[2];
	u8 bufctl4[2];
	
	bufctl3[0] = HSCDTD_CTRL3;
	bufctl3[1] = 0x24;
	err = hscdtd_i2c_write(bufctl3, 2);

	bufctl4[0] = HSCDTD_CTRL4;
	bufctl4[1] = 0x90;
	err = hscdtd_i2c_write(bufctl4, 2);
...
int hscdtd_get_hardware_data(int *xyz)
{
	...
	if (atomic_read(&flgSuspend) == 1)
		return -1;
	hscdtd_activate(1, 1, 10);
	HSCDTD_DELAY(5000);
	ret = hscdtd_get_magnetic_field_data(xyz);
	hscdtd_activate(1, 0, atomic_read(&delay));
	return ret;
}
 ...
 * regulator
 *--------------------------------------------------------------------------*/
static int hscdtd_regulator_enable(struct i2c_client *client)
{
	int err=0;
...
static void hscdtd_regulator_disable(void)
{
	if( sensor_vdd ) {
			...
			const struct i2c_device_id *id)
{
	...
	if(hscdtd_regulator_enable(client)) {
...
static struct of_device_id hscdtd_match_table[] = {
...
static struct i2c_driver hscdtd_driver = {
	...
	.driver		= {
		...
		.owner  = THIS_MODULE,
		.of_match_table = hscdtd_match_table,
	},
